...
def save_prediction(pred_mask,output_dir,file_name):
    ...
    save_mask.putpalette(_palette) # white
    save_mask.save(os.path.join(output_dir,file_name))

def save_object_masks(Seg_Tracker, pred_mask, output_dir, frame_idx, frame_num):
    obj_ids = np.unique(pred_mask)[1:]  # 忽略背景
    user_labels = Seg_Tracker.label_list  # 用户提供的标签列表
    label_list = getattr(Seg_Tracker, 'label_list', [])
    saved_labels = set()  # 避免重复保存
    
    if not label_list:
        label_list = [f"object_{i+1}" for i in range(len(obj_ids))]  # 回退默认名称

    
    for obj_id in obj_ids:
        if obj_id > len(label_list):
            continue  # 跳过无效ID
        label = label_list[obj_id - 1]
        
        # 跳过未命名的物体和重复标签
        if label not in user_labels or label in saved_labels:
            continue
        
        obj_dir = os.path.join(output_dir, label)
        os.makedirs(obj_dir, exist_ok=True)
        
        binary_mask = (pred_mask == obj_id).astype(np.uint8) * 255
        mask_filename = os.path.join(obj_dir, f"{frame_idx + frame_num:05d}.png")
        Image.fromarray(binary_mask, mode='L').save(mask_filename)
...
def tracking_objects_in_video(SegTracker, input_video, frame_length, fps, input_img_seq=None, frame_num=0):
    ...
    if input_video is not None:
        return video_type_input_tracking_new(SegTracker, input_video, io_args, video_name, frame_length)
    elif input_img_seq is not None:
        return img_seq_type_input_tracking(SegTracker, io_args, video_name, imgs_path, fps, frame_num)

def video_type_input_tracking_new(SegTracker, input_video, io_args, video_name, frame_length):
    ...
    with torch.cuda.amp.autocast():
        while cap.isOpened() and frame_idx < frame_length:
            ret, frame = cap.read()
            if not ret:
                break
            ...
            if not isinstance(pred_mask, np.ndarray):
                ...
                if len(non_zero_ids) == 0:
                    ...
                    # 合并所有非零对象
                    binary_mask = np.isin(pred_mask, non_zero_ids).astype(np.uint8) * 255
            pil_mask = Image.fromarray(binary_mask, mode='L')
            pred_list.append(pil_mask)
...
def video_type_input_tracking(SegTracker, input_video, io_args, video_name, frame_num=0):
    ...
    with torch.cuda.amp.autocast():
        while cap.isOpened():
            ...
            save_object_masks(SegTracker, pred_mask, output_mask_dir, frame_idx, frame_num)
