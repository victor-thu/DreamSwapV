...
class SegTracker():
    ...
    def detect_and_seg(self, origin_frame: np.ndarray, grounding_caption, box_threshold, text_threshold, box_size_threshold=1, reset_image=False):
        ...
        captions = grounding_caption.split(', ')
        for caption in captions:
            ...
            if boxes.any():
                ...
                interactive_mask = self.sam.segment_with_box(origin_frame, box, reset_image)[0]
                refined_mask = self.add_mask(interactive_mask)
                self.update_origin_merged_mask(refined_mask)
                self.curr_idx += 1
        ...
        return refined_mask, annotated_frame
