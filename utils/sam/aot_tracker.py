...
import utils.sam.aot.dataloaders.video_transforms as tr
...
class AOTTracker(object):
    def __init__(self, cfg, gpu_id=4):
        self.gpu_id = gpu_id
        self.model = build_vos_model(cfg.MODEL_VOS, cfg).cuda(gpu_id)
        self.model, _ = load_network(self.model, '/mnt/bn/lyl/wwt/wanx_video_pose/utils/sam/ckpt/R50_DeAOTL_PRE_YTB_DAV.pth', gpu_id)
    ...
    def track(self, image):
        ...
        sample = self.transform(sample)
        image = sample[0]['current_img'].unsqueeze(0).float().cuda(self.gpu_id) # edited
...
class AOTTrackerInferEngine(AOTInferEngine):
    def __init__(self, aot_model, gpu_id=4, long_term_mem_gap=9999, short_term_mem_skip=1, max_aot_obj_num=None):
...
def get_aot(args):
    # build vos engine
    engine_config = importlib.import_module('utils.sam.aot.configs.' + 'pre_ytb_dav')
